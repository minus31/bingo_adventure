...
├── snapshots/{revision}-{event}.json # 버전별 공유 상태
...
상태 변경마다 고유 이벤트와 버전별 스냅샷을 새 파일로 저장합니다. 동시에 여러 팀이 제출해도 먼저 저장된 변경을 덮어쓰지 않습니다. 팀별 진행 파일에는 고유 배치, 제출 수, 선두 및 확정 팀, 빙고 수, 시간순 활동 로그가 들어갑니다. 날짜가 바뀌면 새 경로에서 빈 게임으로 시작합니다. 관리자가 전체 초기화를 실행하면 해당 날짜의 영상 원본도 함께 삭제됩니다.
